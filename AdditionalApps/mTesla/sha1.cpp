...
void Sha1Class::writeByte(uint8_t data) {
  ++byteCount;
  addUncounted(data);
}

void Sha1Class::update(const uint8_t* data, uint8_t dataLen) {
...
void Sha1Class::initHmac(const uint8_t* key, int keyLength) {
  ...
  for (i=0; i<BLOCK_LENGTH; i++) {
    writeByte(keyBuffer[i] ^ HMAC_IPAD);
  }
}
...
uint8_t* Sha1Class::resultHmac(void) {
  ...
  reset();
  for (i=0; i<BLOCK_LENGTH; i++) writeByte(keyBuffer[i] ^ HMAC_OPAD);
  for (i=0; i<HASH_LENGTH; i++) writeByte(innerHash[i]);
  return result();
}
Sha1Class Sha1;
